...
Theorem sum_ge_elt :
  ...
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z /\ (i < (length s))%Z ->
  ((get s i) <= (sum1 s))%Z.
(* Why3 intros s h1 i (h2,h3). *)
Proof.
intros s h1 i (h2,h3).
unfold sum1.
(* we divide the sum into two sums *)
generalize (sum_decomp (fun i0 : int => get s i0) 0 i (length s)).
intro decomp.
cut ((0 <= i  <= length s)%Z).
intro bound.
generalize (decomp bound).
intro decomp_inst.
rewrite decomp_inst.
(* exib get s i *)
generalize (sum_left (fun i0 : int => get s i0) i (length s) h3).
intro decomp2.
rewrite decomp2.
(* each of the two sums is positive *)
cut ((0<= sum (fun i0 : int => get s i0) 0 i)%Z).
cut ((0<= sum (fun i0 : int => get s i0) (i + 1) (length s))%Z).
intros.
lia.
(* ((sum (fun i0 : int => get s i0) 0 i >=0)%Z).*)
apply sum_nonneg.
intros.
apply h1.
lia.
(* ((sum (fun i0 : int => get s i0) (i + 1) (length s) >=0)%Z). *)
apply sum_nonneg.
intros.
apply h1.
lia.
