...
Theorem splitRPI :
  ...
  (forall (j:Numbers.BinNums.Z), (b <= j)%Z /\ (j < (k pb))%Z ->
   ~ ((get (get (get (r sw) p) i) j) = Init.Datatypes.true)).
(* Why3 intros k' pb sw h1 h2 p (h3,h4) i (h5,h6). *)
Proof.
intros k' pb sw h1 h2 p cp i ci.
generalize (casesRPI k' pb sw h1 h2 p cp i ci).
intro c.
destruct c.
(* all true *)
exists (k pb).
...
lia.
(*not all true *)
generalize (firstNot k' pb sw h1 h2 p cp i ci H).
intro fnot.
destruct fnot.
exists ((FsetInt.min_elt (Fset.filter (FsetInt.interval 0 (k pb)) (fc sw p i)))).
split.
lia.
split.
destruct H1.
exact H2.
destruct H1.
generalize (notThenNot k' pb sw h1 h2 p cp i ci ((FsetInt.min_elt (Fset.filter (FsetInt.interval 0 (k pb)) (fc sw p i)))) H0 H1).
tauto.
Qed.
