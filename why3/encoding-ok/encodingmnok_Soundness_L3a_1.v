...
(* Why3 intros k' pb sw nw h1 h2 p (h3,h4) i (h5,h6) j (h7,h8). *)
Proof.
intros k' pb sw nw h1 h2 p (h3,h4) i ci j (h7,h8).
(*  X[i][j][l] in hypothesis *)
cut ((j< (k pb))%Z).
intro h9.
generalize (buildLM k' pb sw i j ci (conj h7 h9)).
intro.
destruct H.
rename x0 into l.
destruct H.
destruct H0.
(* R[p][i][j] in hypothesis *)
generalize (L3a1 k' pb sw h1 h2 p  (conj h3 h4) i ci j (conj h7 h8) ).
intro.
(* T[p][i][j][l)  *)
generalize h2.
intro cph2.
...
rewrite <- defn in h4.
generalize (equivTXR k' pb sw HlinkTXR i j p l ci (conj h7 h9) (conj h3 h4) H).
intro.
destruct H3.
generalize (H4 (conj H0 H2)).
intro.
(* conclusion *)
rewrite defn in h4.
cut(num_to_literal pb l = get (get (lm nw) i) j).
intro nH0.
rewrite <- nH0.
apply (L3a2 k' pb sw cph1 cph2 p  (conj h3 h4) i ci j (conj h7 h8) l H H5). 
(* num_to_literal pb l = get (get (lm nw) i) j *)
symmetry.
tauto.
(* j< (k pb)*) 
generalize (L3a0 k' pb sw h1 h2 p  (conj h3 h4) i ci).
cut ((build_nested_witness k' pb sw) = nw).
intro rr.
rewrite rr.
lia.
tauto.
