...
Axiom splitRPI :
  ...
  forall (p:Numbers.BinNums.Z), (0%Z <= p)%Z /\ (p < (length (d pb)))%Z ->
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z /\ (i < k')%Z ->
  exists b:Numbers.BinNums.Z,
  ((0%Z <= b)%Z /\ (b <= (k pb))%Z) /\
  (forall (j:Numbers.BinNums.Z), (0%Z <= j)%Z /\ (j < b)%Z ->
   ((get (get (get (r sw) p) i) j) = Init.Datatypes.true)) /\
  (forall (j:Numbers.BinNums.Z), (b <= j)%Z /\ (j < (k pb))%Z ->
   ~ ((get (get (get (r sw) p) i) j) = Init.Datatypes.true)).

Axiom equivTXR :
  forall (k':Numbers.BinNums.Z), forall (input:problem),
  forall (sw:sat_witness), linkTXR k' input sw ->
  forall (i:Numbers.BinNums.Z) (j:Numbers.BinNums.Z) (p:Numbers.BinNums.Z)
    (l:Numbers.BinNums.Z),
  (0%Z <= i)%Z /\ (i < k')%Z -> (0%Z <= j)%Z /\ (j < (k input))%Z ->
  (0%Z <= p)%Z /\ (p < (n input))%Z -> (0%Z <= l)%Z /\ (l < (m input))%Z ->
...
Axiom atMostOneElementBis :
  forall (k':Numbers.BinNums.Z), forall (input:problem),
  forall (sw:sat_witness),
  atMostOneElement k' input sw /\ linkTXR k' input sw ->
  forall (p:Numbers.BinNums.Z) (i:Numbers.BinNums.Z) (j:Numbers.BinNums.Z)
    (a:Numbers.BinNums.Z) (b:Numbers.BinNums.Z),
  (0%Z <= p)%Z /\ (p < (n input))%Z -> (0%Z <= i)%Z /\ (i < k')%Z ->
  (0%Z <= j)%Z /\ (j < (k input))%Z -> (0%Z <= a)%Z /\ (a < (m input))%Z ->
  (0%Z <= b)%Z /\ (b < (m input))%Z -> ~ (a = b) ->
  ~ ((get (get (get (get (t sw) p) i) j) a) = Init.Datatypes.true) \/
  ~ ((get (get (get (get (t sw) p) i) j) b) = Init.Datatypes.true).

Axiom buildLMOK :
  forall (k':Numbers.BinNums.Z), forall (pb:problem),
  forall (sw:sat_witness), problem_valid pb -> is_sat_witness k' pb sw ->
  forall (i:Numbers.BinNums.Z) (j:Numbers.BinNums.Z),
  (0%Z <= i)%Z /\ (i < k')%Z -> (0%Z <= j)%Z /\ (j < (k pb))%Z ->
  exists l:Numbers.BinNums.Z,
  ((0%Z <= l)%Z /\ (l < (m pb))%Z) /\
  ((get (get (get (x sw) i) j) l) = Init.Datatypes.true).

Parameter build_nested_witness:
  Numbers.BinNums.Z -> problem -> sat_witness -> nested_witness.
...
Axiom buildLM :
  ...
  ((0%Z <= l)%Z /\ (l < (m input))%Z) /\
  ((get (get (get (x sw) i) j) l) = Init.Datatypes.true) /\
  ((get (get (lm (build_nested_witness k' input sw)) i) j) =
   (num_to_literal input l)).
...
(* Why3 assumption *)
Definition srpij (sw:sat_witness) :
    Numbers.BinNums.Z ->
    Numbers.BinNums.Z -> Numbers.BinNums.Z -> Numbers.BinNums.Z :=
  fun (p:Numbers.BinNums.Z) => fun (i:Numbers.BinNums.Z) => fc1 sw p i.
...
Axiom buildRPI :
  ...
  forall (p:Numbers.BinNums.Z) (i:Numbers.BinNums.Z),
  (0%Z <= p)%Z /\ (p < (n input))%Z -> (0%Z <= i)%Z /\ (i < k')%Z ->
  ((get (get (rpi (build_nested_witness k' input sw)) p) i) =
   (sum (srpij sw p i) 0%Z (k input))).

Axiom L1 :
  forall (k':Numbers.BinNums.Z), forall (pb:problem),
  forall (sw:sat_witness), problem_valid pb -> is_sat_witness k' pb sw ->
  nested_witness_valid k' pb (build_nested_witness k' pb sw).

Axiom L2 :
  forall (k':Numbers.BinNums.Z), forall (pb:problem),
  forall (sw:sat_witness), problem_valid pb -> is_sat_witness k' pb sw ->
  forall (p:Numbers.BinNums.Z), forall (i:Numbers.BinNums.Z),
  (0%Z <= p)%Z /\ (p < (length (d pb)))%Z -> (0%Z <= i)%Z /\ (i < k')%Z ->
  ((get (get (rpi (build_nested_witness k' pb sw)) p) i) <= (k pb))%Z.
...
Axiom L3a1 :
  ...
  (0%Z <= j)%Z /\
  (j < (get (get (rpi (build_nested_witness k' pb sw)) p) i))%Z ->
  ((get (get (get (r sw) p) i) j) = Init.Datatypes.true).
...
Axiom L3a2 :
  ...
  (0%Z <= j)%Z /\
  (j < (get (get (rpi (build_nested_witness k' pb sw)) p) i))%Z ->
  forall (l:Numbers.BinNums.Z), (0%Z <= l)%Z /\ (l < (m pb))%Z ->
  ((get (get (get (get (t sw) p) i) j) l) = Init.Datatypes.true) ->
  set.Fset.mem (num_to_literal pb l) (get (d pb) p).
...
Axiom L3a3 :
  ...
  ((0%Z <= l)%Z /\ (l < (m pb))%Z) /\
  ((num_to_literal pb l) =
   (get (get (lm (build_nested_witness k' pb sw)) i) j)).

Axiom L3a :
  forall (k':Numbers.BinNums.Z), forall (pb:problem),
  forall (sw:sat_witness),
  let nw := build_nested_witness k' pb sw in
  problem_valid pb -> is_sat_witness k' pb sw ->
  forall (p:Numbers.BinNums.Z), (0%Z <= p)%Z /\ (p < (length (d pb)))%Z ->
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z /\ (i < k')%Z ->
  forall (j:Numbers.BinNums.Z),
  (0%Z <= j)%Z /\ (j < (get (get (rpi nw) p) i))%Z ->
  set.Fset.mem (get (get (lm nw) i) j) (get (d pb) p).

Axiom L3b0 :
  forall (k':Numbers.BinNums.Z), forall (pb:problem),
  forall (sw:sat_witness), problem_valid pb -> is_sat_witness k' pb sw ->
  forall (p:Numbers.BinNums.Z), forall (i:Numbers.BinNums.Z),
  forall (j:Numbers.BinNums.Z), (0%Z <= p)%Z /\ (p < (length (d pb)))%Z ->
  (0%Z <= i)%Z /\ (i < k')%Z -> (0%Z <= j)%Z /\ (j < (k pb))%Z ->
  ((get (get (get (r sw) p) i) j) = Init.Datatypes.true) ->
  (0%Z <= j)%Z /\
  (j < (get (get (rpi (build_nested_witness k' pb sw)) p) i))%Z.
...
Theorem L3b :
  ...
  let nw := build_nested_witness k' pb sw in
  problem_valid pb -> is_sat_witness k' pb sw ->
  forall (p:Numbers.BinNums.Z), forall (l:literal),
  (0%Z <= p)%Z /\ (p < (length (d pb)))%Z -> set.Fset.mem l (get (d pb) p) ->
  exists i:Numbers.BinNums.Z,
  exists j:Numbers.BinNums.Z,
  ((0%Z <= i)%Z /\ (i < k')%Z) /\
  ((0%Z <= j)%Z /\ (j < (get (get (rpi nw) p) i))%Z) /\
  (l = (get (get (lm nw) i) j)).
(* Why3 intros k' pb sw nw h1 h2 p l (h3,h4) h5. *)
Proof.
intros k' pb sw nw h1 h2 p l cp h5.
generalize h2.
intro cph2.
destruct cph2 as [Hsat_witness_invariant [HatLeastOneElement [HatMostOneElement [Hnested [HlinkTXR [HbuildTMem HbuildTNotMem]]]]]].
unfold buildTMem in HbuildTMem.
(* Get the index of the literal *)
generalize (literal_to_num pb l p h1 cp h5).
intro.
destruct H.
destruct H.
(* Get T *)
generalize h1.
intro cph1.
unfold problem_valid in cph1.
destruct cph1 as [kpos [defm [mpos [defn [npos [dkDNF [doverL bij]]]]]]].
rewrite <- defn in cp.
rewrite <- H0 in h5.
generalize (HbuildTMem x0 p H cp h5).
intro.
destruct H1 as [i0 [j0 [ci0 [cj0 Tij0]]]].
exists i0.
exists j0.
split.
exact ci0.
(* Got T, get R and X *)
generalize (equivTXR k' pb sw HlinkTXR i0 j0 p x0 ci0 cj0 cp H).
intro.
rewrite H1 in Tij0.
destruct Tij0 as [Xij0 Rij0].
clear H1.
split.
(* Got R, so index is smaller than rpi *)
rewrite defn in cp.
apply (L3b0 k' pb sw h1 h2 p i0 j0 cp ci0 cj0).
exact Rij0.
(* Got X, so get lm *)
generalize (buildLM k' pb sw i0 j0 ci0 cj0).
intro.
destruct H1.
destruct H1.
destruct H2.
unfold atMostOneElement in HatMostOneElement.
generalize (HatMostOneElement i0 j0 x0 x1 ci0 cj0 H H1).
intro.
cut (x0 <> x1 \/ x0=x1).
intro.
destruct H5.
