...
Theorem L3b0 :
  ...
  (0%Z <= j)%Z /\
  (j < (get (get (rpi (build_nested_witness k' pb sw)) p) i))%Z.
(* Why3 intros k' pb sw h1 h2 p i j (h3,h4) (h5,h6) (h7,h8) h9. *)
Proof.
intros k' pb sw h1 h2 p i j cp ci cj h9.
unfold is_sat_witness in h2.
destruct h2 as [Hsat_witness_invariant [HatLeastOneElement [HatMostOneElement [Hnested [HlinkTXR [HbuildTMem HbuildTNotMem]]]]]].
generalize (splitRPI k' pb sw h1 Hnested p cp i ci).
intro.
destruct H.
cut (x0= get (get (rpi (build_nested_witness k' pb sw)) p) i).
intro.
destruct H as [cx0 [Hb Ha]].
(* bound *) 
assert ((0 <= j < x0)%Z \/ (x0 <= j < (k pb))%Z) by lia.
destruct H.
rewrite H0 in H.
exact H.
(* contradiction *)
...
intro.
tauto.
(* x0 = get (get (rpi (build_nested_witness pb sw)) p) i *)
rewrite buildRPI.
(* Split the sum in two part : only 1, only 0 *)
rewrite (sum_decomp (srpij sw p i) 0 x0 (k pb)).
cut (sum (srpij sw p i) 0 x0 = x0).
cut ((sum (srpij sw p i) x0 (k pb) = 0)%Z).
lia.
(* sum (srpij sw p i) x0 (k pb) = 0%Z *)
apply (sum_zero (srpij sw p i) x0 (k pb)).
unfold srpij.
intros.
generalize (fc'def1 sw p i i0).
intro.
destruct H.
destruct H2.
destruct H1.
...
apply (H3 i0).
lia.
(* sum (srpij sw p i) 0 x0 = x0 *)
apply sum_one.
lia.
intros.
generalize (fc'def1 sw p i j0).
intro.
destruct H.
destruct H2.
destruct H1.
...
lia.
lia.
unfold problem_valid in h1.
destruct h1 as [kpos [defm [mpos [defn [npos [dkDNF [doverL bij]]]]]]].
lia.
lia.
Qed.