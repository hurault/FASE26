...
intro r1.
rewrite r1.
generalize (cardinal_map (fun v : feature => Pos v) s0).
generalize (cardinal_nonneg (Fset.map (fun v : feature => Pos v) s0)).
lia.
(* general case *)
intros.
...
(* ~ mem t s0 *)
unfold card_map_pos.
generalize (add_map (fun v : feature => Pos v) s0 t H0).
intro r.
rewrite r.
unfold card_map_pos in H.
...
intro r1.
rewrite r1.
generalize (cardinal_add (Pos t) (Fset.map (fun v : feature => Pos v) s0)).
intro.
destruct H3.
cut (~ Fset.mem (Pos t) (Fset.map (fun v : feature => Pos v) s0)).
intro.
generalize (H4 H5).
lia.
(* ~ Fset.mem (Pos t) (Fset.map (fun v : feature => Pos v) s0) *)
apply (not_mem_map_pos s0 t H0).
tauto.
Qed.