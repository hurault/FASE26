...
Parameter build_satisfiable_witness:
  Numbers.BinNums.Z -> problem -> nested_witness -> sat_witness.

Axiom size :
  forall (k':Numbers.BinNums.Z), forall (input:problem),
  forall (nw:nested_witness),
  sat_witness_valid k' input (build_satisfiable_witness k' input nw).

Axiom buildX :
  forall (k':Numbers.BinNums.Z), forall (input:problem),
  forall (nw:nested_witness),
  forall (i:Numbers.BinNums.Z) (j:Numbers.BinNums.Z) (l:Numbers.BinNums.Z),
  (0%Z <= i)%Z /\ (i < k')%Z -> (0%Z <= j)%Z /\ (j < (k input))%Z ->
  (0%Z <= l)%Z /\ (l < (m input))%Z ->
  ((get (get (get (x (build_satisfiable_witness k' input nw)) i) j) l) =
   Init.Datatypes.true) <->
  ((get (get (lm nw) i) j) = (num_to_literal input l)).
...
Axiom buildR :
  ...
  (0%Z <= i)%Z /\ (i < k')%Z -> (0%Z <= j)%Z /\ (j < (k input))%Z ->
  (0%Z <= p)%Z /\ (p < (n input))%Z ->
  ((get (get (get (r (build_satisfiable_witness k' input nw)) p) i) j) =
   Init.Datatypes.true) <->
  (j < (get (get (rpi nw) p) i))%Z.
...
Axiom L2 :
  ...
  forall (nw:nested_witness), problem_valid input ->
  is_nested_witness k' input nw ->
  atLeastOneElement k' input (build_satisfiable_witness k' input nw).

Axiom L3 :
  forall (k':Numbers.BinNums.Z), forall (input:problem),
  forall (nw:nested_witness), problem_valid input ->
  is_nested_witness k' input nw ->
  atMostOneElement k' input (build_satisfiable_witness k' input nw).

Axiom L4 :
  forall (k':Numbers.BinNums.Z), forall (input:problem),
  forall (nw:nested_witness), problem_valid input ->
  is_nested_witness k' input nw ->
  nested k' input (build_satisfiable_witness k' input nw).

Axiom L5 :
  forall (k':Numbers.BinNums.Z), forall (input:problem),
  forall (nw:nested_witness), problem_valid input ->
  is_nested_witness k' input nw ->
  linkTXR k' input (build_satisfiable_witness k' input nw).
...
Theorem L6 :
  ...
  forall (nw:nested_witness), problem_valid input ->
  is_nested_witness k' input nw ->
  buildTMem k' input (build_satisfiable_witness k' input nw).
(* Why3 intros k' input nw h1 h2. *)
Proof.
intros k' input nw h1 h2.
generalize h2.
intro cph2.
destruct cph2 as [nwv [rpiinfk lmrdefd]].
generalize rpiinfk.
intro rpiinfk_inst.
generalize h1.
intro pinv.
...
unfold is_bijection in bij.
destruct bij as [dompos [image [surjective [injective cardeq]]]].
unfold buildTMem.
intros.
rewrite defn in H0.
generalize (dpInlm k' input nw pinv h2 p (num_to_literal input l) H0 H1).
intro tt.
destruct tt.
destruct H2.
rename x0 into i.
...
destruct H2.
destruct H3.
generalize (rpiinfk_inst p i H0 H2). 
lia.
(* get (get (get (get (t (build_satisfiable_witness input nw)) p) i) j) l = true *)
apply buildT.
lia.
(* begin (0 <= j < k input)%Z *)
destruct H2.
...
lia.
split.
(* get (get (get (x (build_satisfiable_witness input nw)) i) j) l = true *)
apply buildX.
lia.
(* begin (0 <= j < k input)%Z *)
destruct H2.
...
symmetry.
tauto.
(* get (get (get (r (build_satisfiable_witness input nw)) p) i) j = true *)
apply buildR.
lia.
(* begin (0 <= j < k input)%Z *)
destruct H2.
