...
(* Why3 intros a b p (i,((h1,h2),h3)) i1 (h4,h5). *)
Proof.
...
generalize (H0 i).
intros.
assert (H2contra : ~ ((FsetInt.min_elt (Fset.filter (FsetInt.interval a b) p) <= i)%Z) -> ~ (Fset.mem i (Fset.filter (FsetInt.interval a b) p))) by (intros HQ HP; apply HQ; apply H2; exact HP).
cut (~ (FsetInt.min_elt (Fset.filter (FsetInt.interval a b) p) <= i)%Z).
intro.
apply H2contra in H3.
generalize (filter_def (FsetInt.interval a b) p).
intro.
generalize (H4 i).
intro.
destruct H5.
assert (H6contra : ~ (Fset.mem i (Fset.filter (FsetInt.interval a b) p)) -> ~ (Fset.mem i (FsetInt.interval a b) /\ p i = true )) by (intros HQ HP; apply HQ; apply H6; exact HP).
apply H6contra in H3.
apply not_and_or in H3.
destruct H3.
generalize (interval_def a b i).
intro.
generalize (min_filter_interval_1 a b p not_all).
intro.
destruct H7.
assert (H9contra : ~ ( Fset.mem i (FsetInt.interval a b)) -> ~ ((a <= i < b)%Z) ) by (intros HQ HP; apply HQ; apply H9; exact HP).
apply H9contra in H3.
lia.
exact H3.
lia.
Qed.
